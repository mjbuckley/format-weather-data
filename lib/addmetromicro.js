...
function addMetroMicro(stationsObj) {
    ...
    if (zipToCbsaMap[zip]) {
      ...
      for (let i = 0; i < cbsaArr.length; i++) {
        ...
        if (cbsaCount[cbsaNum]) {
          let number = cbsaCount[cbsaNum][0];
          number ++;
          cbsaCount[cbsaNum][0] = number;
          cbsaCount[cbsaNum][1].push(stationsObj[station]["city"]);
        } else {
          const city = stationsObj[station]["city"];
          newArr = [1, [city]];
          cbsaCount[cbsaNum] = newArr;
        }
      }
    }
  }
  ...
  // Remove cbsa ids from cbsaCount if their stations are all in the same city
  for (let id in cbsaCount) {
    const cities = cbsaCount[id][1]; //array of cities in cbsa
    const firstCity = cities[0];

    // Looks a bit funny but returns true if all cities match first city
    if (!cities.some(city => city !== firstCity)) {
      delete cbsaCount[id];
    }
  }
    ...
    if (stationsObj[station]["area"].length > 0) { // Check if station has any cbsa codes
      ...
      for (let i = 0; i < areaArr.length; i++) { // loop through current codes array and change codes to names
        ...
        if (cbsaMap[cbsaValue]) { // check if cbsa code is in cbsa to city name map
          ...
          // NEW STUFF Below
          let cbsaStates = cbsaMap[cbsaValue]["states"];
          let combinedArea = cbsaName + ", " + cbsaStates;
          // END NEW (but changed replacement beloew from cbsaName to combinedArea)


          replacement.push(combinedArea); // Add name to replacement array
        } else {
