...
function addMetroMicro(stationsObj) {
  ...
  for (let station in stationsObj) {
    ...
    if (zipToCbsaMap[zip]) {
      ...
      for (let i = 0; i < cbsaArr.length; i++) {
        ...
        stationsObj[station]["location"]["area"].push(cbsaNum); // Add cbsa to station
  ...
  for (let station in stationsObj) {
    if (stationsObj[station]["location"]["area"].length > 0) {
      let areaArr = stationsObj[station]["location"]["area"];
      for (let id of areaArr) {
        if (cbsaCount[id]) {
          stationsObj[station]["location"]["sharedarea"].push(id);
        }
      }
    }
  }
  ...
  for (let station in stationsObj) {
    if (stationsObj[station]["location"]["sharedarea"].length > 0) { // Check if station has any cbsa codes
      let sharedAreaArr = stationsObj[station]["location"]["sharedarea"]; // grab array of current codes. Ex: [23432, 23433]
      let replacement = []; // Array to hold cbsa names. Will replace current array with id numbers
      ...
      // Replace current area array with replacement array containing metro/micro names
      stationsObj[station]["location"]["sharedarea"] = replacement;
    }
  }
